---
title: "Batch Campaign API"
description: "Place outbound AI voice calls to a list of contacts with retries, scheduling, and business-hours gating. One API key, seven endpoints."
---
...
<Accordion title="POST /campaign/create — Create a campaign" icon="plus" defaultOpen>
...
<ParamField body="retry_intervals" type="number[]" default="[]">
...
<ResponseField name="campaign_id" type="string (UUID)">Identifier for subsequent calls.</ResponseField>
<ResponseField name="campaign_name" type="string" />
<ResponseField name="status" type="string">`draft` or `scheduled` (when `scheduled_at` was set).</ResponseField>
<ResponseField name="total_contacts" type="number" />
<ResponseField name="business_hours" type="BusinessHours | null">Canonicalized config — days are sorted and deduped, windows are sorted by start.</ResponseField>
...
<ParamField body="campaign_id" type="string (UUID)" required />
...
<ResponseField name="campaign_name" type="string" />
<ResponseField name="status" type="string">One of: `draft`, `scheduled`, `warmup`, `in-progress`, `paused`, `completed`, `cancelled`, `dormant`. See [Lifecycle](#lifecycle).</ResponseField>
<ResponseField name="total_contacts" type="number" />
<ResponseField name="completed_calls" type="number">Completed call attempts (success + failure).</ResponseField>
<ResponseField name="successful_calls" type="number" />
<ResponseField name="failed_calls" type="number" />
<ResponseField name="started_at" type="string | null" />
<ResponseField name="business_hours" type="BusinessHours | null" />
...
<ResponseField name="contact_status_breakdown" type="object">
  ...
  <Expandable title="Breakdown">
    <ResponseField name="pending" type="number">Awaiting first dial.</ResponseField>
    <ResponseField name="calling" type="number">Currently in an active call.</ResponseField>
    <ResponseField name="completed" type="number">Reached a terminal success state.</ResponseField>
    <ResponseField name="failed" type="number">Permanent failure (after retries exhausted).</ResponseField>
    <ResponseField name="retry_scheduled" type="number">Awaiting next retry attempt.</ResponseField>
    <ResponseField name="cancelled" type="number">Cancelled by user/system.</ResponseField>
  </Expandable>
</ResponseField>
<ResponseField name="max_retries" type="number" />
<ResponseField name="retry_intervals" type="number[]" />
